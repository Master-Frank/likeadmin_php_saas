...
平台端入口校验 `project.http_host`：与浏览器地址栏主机不一致时会返回「平台端入口域名错误」。用 `http://127.0.0.1:8080/platform/` 访问时，该项应写成 `127.0.0.1:8080`；不限域名则置空。访问站点根 `/` 时，若 Host 等于 `http_host`（或该项为空），Go 与历史 PHP `index.php` 一样 302 到 `/platform/`。

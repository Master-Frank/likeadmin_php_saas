...
func FileLists(c *gin.Context) {
	...
	db = filesvc.ApplyFileCID(db, &model.FileCate{}, q.Params, 0)
	var count int64
	db.Count(&count)
	var rows []model.File
...
func FileDelCate(c *gin.Context) {
	...
	if bootstrap.DB.Where("id = ? AND delete_time IS NULL", id).First(&cate).Error != nil {
...
func uploadSave(c *gin.Context, typ int, dir, scene string) {
	if msg := filesvc.UploadCateOK(bootstrap.DB, &model.FileCate{}, filesvc.UploadCID(c), 0); msg != "" {
		response.Fail(c, msg)
		return
	}

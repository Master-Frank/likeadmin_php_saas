...
func WorkbenchIndex(c *gin.Context) {
	...
	todayNew = workbench.CachedCount(workbench.TenantTodayKey(now), func() int64 {
		var n int64
		bootstrap.RequestReadDB(c).Model(&model.Tenant{}).Where("delete_time IS NULL AND create_time >= ?", todayStart).Count(&n)
		return n
	})
	totalNew = workbench.CachedCount(workbench.TenantTotalKey(), func() int64 {
		var n int64
		bootstrap.RequestReadDB(c).Model(&model.Tenant{}).Where("delete_time IS NULL").Count(&n)
		return n
	})
